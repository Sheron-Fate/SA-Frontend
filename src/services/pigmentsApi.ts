import type { Pigment, PigmentsResult, CreatePigmentRequest, UpdatePigmentRequest } from '../types/pigment'
...
export const getPigments = async (filters: PigmentFiltersRequest = {}): Promise<PigmentsResult> => {
  ...
  // API_BASE_URL для Tauri = "http://172.20.10.3:8080", для веба = "/"
  // Для Tauri: http://172.20.10.3:8080/api/pigments
  // Для веба: /api/pigments (будет проксироваться)
  // Убираем /api из baseUrl, если он там есть
...
export const getPigmentById = async (id: string): Promise<{ pigment: Pigment }> => {
  ...
  // Убираем /api из baseUrl, если он там есть
  let baseUrl = API_BASE_URL === '/' ? '' : API_BASE_URL.replace(/\/api\/?$/, '')
  const url = `${baseUrl}/api/pigments/${id}`
...
// Создание пигмента (только для модератора)
export const createPigment = async (data: CreatePigmentRequest): Promise<{ pigment: Pigment }> => {
  ...
  let baseUrl = API_BASE_URL === '/' ? '' : API_BASE_URL.replace(/\/api\/?$/, '')
  const url = `${baseUrl}/api/pigments`
...
// Обновление пигмента (только для модератора)
export const updatePigment = async (id: number, data: UpdatePigmentRequest): Promise<{ pigment: Pigment }> => {
  const state = store.getState()
  const token = state.auth.accessToken
  ...
  const response = await httpClient.instance.put(url, data, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
...
// Удаление пигмента (только для модератора)
export const deletePigment = async (id: number): Promise<void> => {
  const state = store.getState()
  const token = state.auth.accessToken

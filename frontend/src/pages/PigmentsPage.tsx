...
import { setColor, setDateRange, setSearch, getPigmentsList } from "../features/filters/filtersSlice";

const PigmentsPage: FC = () => {
  const navigate = useNavigate()
  const dispatch = useAppDispatch()
  const { search, color, dateRange, pigments, loading } = useAppSelector(selectFilters)

  // Загружаем пигменты при монтировании компонента
  useEffect(() => {
    dispatch(getPigmentsList())
  }, [dispatch])
